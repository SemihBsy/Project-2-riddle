...
router.get("/seed", (req, res) => {
    ...
    const startRiddles = [
      ...
      // Delete all riddles
      Riddle.deleteMany({}, (err, createdRiddles) => {
        // Seed Starter Riddles
        Riddle.create(startRiddles, (err, createdRiddles) => {
          // send created riddles as response to confirm creation
          res.json(createdRiddles);
        });
      });
});
...
router.get("/", (req, res) => {
    Riddle.find({}, (err, riddles) => {
      let sortedRiddles = riddles.reverse()
      res.render("riddles/index.ejs", { riddles: sortedRiddles });
    });
  });
